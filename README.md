...
10개 스위트 236개: 파싱(45) + 유틸(22) + 통합 조회(17) + 개정비교·구조화응답(14) + 별칭·정규화(17) + 시점 판단(27) + 인용 검증(32) + 엣지케이스(12) + MCP 프로토콜(28, stdout 순수성 포함) + HTTP(22, Streamable HTTP·Bearer 인증·stateless).
실제 HTML 픽스처 기반이라 국내/해외/CI 어디서든 통과해야 정상.

`npm pack --dry-run`으로 약 52kB tarball과 필수 소스·테스트 23개 파일 포함 여부를 검증합니다.
MCP JSON-RPC 핸드셰이크에서는 도구 9개와 stdout 무오염을 확인합니다.
